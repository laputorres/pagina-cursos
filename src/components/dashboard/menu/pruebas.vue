<template>
  <header class="flex flex-wrap md:justify-start md:flex-nowrap max-sm:justify-center z-50 w-full text-sm">
      <nav class="mt-2 max-sm:mt-2 fixed z-10 max-w-[97vw] md:h-[8vh] w-full   mx-2 max-sm:px-0 max-sm:mx-0 py-3 px-4 md:flex md:items-center max-sm:flex max-sm:w-full md:justify-between md:py-0 md:px-6 lg:px-8 xl:mx-auto dark:bg-gray-800 dark:border-gray-700"
          aria-label="Global" id="nav">
          <div class="flex items-center max-sm:px-4 max-sm:w-[85vw] justify-between">
          ...
          <div v-if="navbarOpen || !isMobile" class="w-full ">
              <div
                  class="flex flex-col  max-sm:bg-white gap-y-4  gap-x-0 lg:ml-20  mt-5 md:flex-row md:items-center md:justify-between md:gap-y-0 md:gap-x-7 md:mt-0 md:ps-7">
                  <div class="nav flex max-sm:flex-col gap-10 max-sm:shadow-2xl">
                      <router-link to="/all-categories"
                          class="font-medium text-custom1-light md:py-6 dark:text-blue-500" aria-current="page">{{
                      $t('allcategories') }}</router-link>
                      <a class="font-medium max-sm:w-full text-gray-500 hover:text-gray-400 md:py-6 dark:text-gray-400 dark:hover:text-gray-500"
                          href="#">Account</a>
...
export default {
  ...
  setup() {
      ...
      const containerClass = ref('bg-white');
